    ...
    if (isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true) {
        ...
        if ($_POST["type"] == "update") {
            ...
            if (isset($_SESSION["admin"]) && $_SESSION["admin"] === true) {
                ...
                $res1 = pg_query($link, "UPDATE vex_product SET code = '$code', product_name = '$productName' WHERE product_id = $productId");
            } else {
                //Make sure user can only change their own pages
                $res1 = pg_query($link, "UPDATE vex_product SET code = '$code', product_name = '$productName' WHERE product_id = $productId AND user_id = $user_id");
            }
        } else if ($_POST["type"] == "save") {
            //if the request type is save, insert new page into database
            $date = date('Y-m-d h:i:s');
            $res1 = pg_query($link, "INSERT INTO vex_product (user_id, product_name, code, create_time) VALUES ($user_id, '$productName','$code','$date')");
        }
