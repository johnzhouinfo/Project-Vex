...
$(".drop").on("load", function () {
     ...
     */
    $('.element-style, .element-attribute').off('focusin').on('focusin', setOriginalValue);

    $('.element-style').off('change').on('change', changeStyleAttribute);

    $('.element-attribute').off('change').on('change', changeAttribute);
...
function changeStyleAttribute(event) {
    ...
    } else if ((attr === "left" || attr === "right" || attr === "top" || attr === "bottom" || attr === "width"
        || attr === "height" || attr === "min-width" || attr === "min-height" || attr === "max-width" || attr === "max-height"
        || attr === "margin-top" || attr === "margin-right" || attr === "margin-bottom" || attr === "margin-left"
        || attr === "padding-top" || attr === "padding-right" || attr === "padding-bottom" || attr === "padding-left"
        || attr === "border-width" || attr === "border-top-left-radius" || attr === "border-top-right-radius"
        || attr === "border-bottom-left-radius" || attr === "border-bottom-right-radius") && $.isNumeric(this.value)) {

        $(element).css(attr, this.value + "px");
    } else {
...
/**
 * Resize col/row of the table
 */
$("#table-resize-btn").on("click", function () {
    old_data = $(".drop").contents().find("body").clone();
    var $oldTable = $(element);
    var $table = $oldTable.clone().empty();
    var width = $('#table-col').val();
    var height = $('#table-row').val();
    var theadSize = $($oldTable.get(0).tHead).length;
    var $thead = $('<thead>');
    var row = 0;
    for (; row < theadSize; row++) {
        var $tr = $('<tr>');
        for (var column = 0; column < width; column++) {
            var $th = $('<th>');
            $th = getCellValue($oldTable, 'th', row, column);
            $tr.append($th);
        }
        $thead.append($tr);
    }
    ...
    for (; row < height; row++) {
        ...
        for (var column = 0; column < width; column++) {
            var $td = $('<td>');
            $td = getCellValue($oldTable, 'td', row, column);
            $tr.append($td);
        }
        $tbody.append($tr);
    }
...
function getCellValue($table, tag, row, col) {
    var $trs = $table.find('tr');
    return $trs.length > row && $($trs[row]).find(tag).length > col
        ? $($($trs[row]).find(tag)[col]).clone() : ("<" + tag + ">cell" + row + "," + col + "</" + tag + ">");
}
