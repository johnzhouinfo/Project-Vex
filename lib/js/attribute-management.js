...
function loadStyleAttribute(styleAttributes) {
    ...
    for (var i = 0; i < styleList.length; i++) {
        ...
        if (styleList[i].name === "background-image") {
            $("#style-" + styleList[i].name).val(styleList[i].value.slice(4, -1));
...
function resetData() {
    ...
    $("#vex-toolbar:input:not(#style-opacity,#style-background-color,#style-color,#style-text-decoration-color,#style-border-color)").val("");
    $("#style-opacity").val(1);
    ...
    if (this.files && this.files[0]) {
        ...
        FR.addEventListener("load", function (e) {
            if ($(event.target).attr("attr-type") === "attribute") {
                $(element).attr($(event.target).attr("data-type"), e.target.result);
            } else {
                $(element).css($(event.target).attr("data-type"), "url(\"" + e.target.result + "\"");
            }
        });
