...
$(function () {
    ...
    function dragStartInnerHandler(event) {
        ...
        console.log(old_reffered_item);
        // pre_reffered_element = $('#select-box');
        // console.log(pre_reffered_element.previousSibling);
    ...
    $('.drop').load(function () {
        ...
        $(frameWindow.document).find('body,html').on('drop', function (event) {
            ...
            try {
                ...
                var location, added_item, referred_item;
                if ($(frameWindow.document).find(checkDiv).get(0).previousSibling != null){
                    referred_item = $(frameWindow.document).find(checkDiv).get(0).previousSibling;
                    added_item = referred_item.nextSibling;
                    location = "after";
                }
                else if ($(frameWindow.document).find(checkDiv).get(0).nextSibling != null){
                    referred_item = $(frameWindow.document).find(checkDiv).get(0).nextSibling;
                    added_item = referred_item.previousSibling;
                    location = "before"
                }
                // console.log($(frameWindow.document).find(checkDiv).get(0).parentElement);
                afterdrop(referred_item, added_item, location, old_reffered_item, old_location)
            } catch (e) {
                console.log(e);
            }
        });
        ...
        $(frameWindow.document).contents().on("click", function (event) {
            ...
            // console.log(selectTarget.previousSibling);
            // console.log(selectTarget.nextSibling);
            if (selectTarget.previousSibling != null){
                old_reffered_item = selectTarget.previousSibling;
                old_location = "after"
                // console.log(old_reffered_item);
            }
            else {
                old_reffered_item = selectTarget.nextSibling;
                old_location = "before"
                // console.log(old_reffered_item);
            }
    ...
    var DragDropFunctions =
        {
            ...
            PositionContextMarker: function ($contextMarker, $element) {
                ...
                $contextMarker.css({
                    ...
                    top: (rect.top + $($(".drop").get(1).contentWindow).scrollTop() - 2) + "px",
                    left: (rect.left + $($(".drop").get(1).contentWindow).scrollLeft() - 2) + "px"
                });
